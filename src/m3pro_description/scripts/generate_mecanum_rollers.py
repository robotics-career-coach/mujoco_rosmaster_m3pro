#!/usr/bin/env python3
"""
Generates passive-roller mecanum wheel geometry and splices it into m3pro.xml.
...
# Geometry shared by all 4 wheels (must match the existing cylinder in m3pro.xml).
...
def splice(xml_text, hub_radius, a, step):
    # 1. Drop cone="elliptic" -- only existed to support the anisotropic <pair>
    #    friction overrides being removed below.
    new_text, n = re.subn(r'\s*cone="elliptic"', "", xml_text, count=1)
    if n != 1:
        raise SystemExit('could not find cone="elliptic" in <option> to remove')
    xml_text = new_text
    ...
    # 5. Update the wheel-layout doc comment to describe the new approach + caveat.
    old_comment = (
        "      <!--\n"
        ...
        "\n"
        "        Mecanum X-pattern via anisotropic friction (condim=4, elliptic cone).\n"
        "      -->"
    )
    new_comment = (
        "      <!--\n"
        ...
        "        real ROSMASTER M3 Pro's physical roller wheels, and the resulting effective rolling\n"
        f"        radius is slightly under the nominal {WHEEL_RADIUS} m used in controllers.yaml's\n"
        "        kinematics. Flagged for future calibration against real hardware. NUM_ROLLERS is a\n"
        "        deliberate smoothness/grip-authority trade-off -- see the design note in the\n"
        "        generator script and CLAUDE.md before changing it.\n"
        "      -->"
    )
    if old_comment not in xml_text:
        raise SystemExit("could not find the wheel-layout doc comment to update")
    xml_text = xml_text.replace(old_comment, new_comment, 1)
...
def main():
    ...
    parser.add_argument(
        "--mjcf-path",
        default=str(Path(__file__).parent.parent / "mjcf" / "m3pro.xml"),
        help="Path to m3pro.xml (default: the package's own mjcf/m3pro.xml)",
    )
    ...
    if "wheel_contact" in xml_text or "_roller0" in xml_text:
        raise SystemExit(
            ...
            f"Run `git checkout -- {mjcf_path}` first if you want to regenerate."
        )
